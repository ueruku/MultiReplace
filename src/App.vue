<template>
  <div id="app" style="width: 80%; margin: 0px auto;">
    ...
    <div class="input">
      ...
      <hr />
      <input type="checkbox" v-model="cb_wordDelete" />
...
const textType = ref("TSV");
const cb_wordDelete = ref(false);
...
const replacedText = computed((): string => {
  let table: string[][] = [];
  if (textType.value === "TSV")
    table = combartTsvToTable(replacementTable.value);
  if (textType.value === "CSV")
    table = combartCsvToTable(replacementTable.value);

  // 余計な空行を削除
  table = deleteBlankRowFromTable(table);
  if (table.length === 0) return "--置換表が未設定です--";

  // 末尾の空白を削除
  table = deleteTrailingBlankFromTable(table);

  // １行のカラム数は2つにする(置換前と置換後の2行のみ)
  table = table.map(row => resizeArray(row, 2, ""));
  ...
  let replaced = replaceTextByWords(text.value, table);
  if (text.value === replaced) return "--置換対象の文字列がありません--";
...
/** 2次元配列から空行を削除 */
const deleteBlankRowFromTable = (table: string[][]): string[][] => {
  return table.filter(row => row.some(col => col !== ""));
}

/** 2次元配列から末尾が空文字の項を削除 */
const deleteTrailingBlankFromTable = (table: string[][]): string[][] => {
...
/** CSV→2次元配列 */
const combartCsvToTable = (text: string): string[][] => {
  let csvRows: string[][] = [];
  let csvRow: string[] = [];
...
/** TSV→2次元配列 */
const combartTsvToTable = (tsv: string): string[][] => {
  return tsv.split("\n").map(row => row.split("\t"));
}
 ...
 * lengthにマイナスを入れるとUB。
 */
const resizeArray = (arr: any[], length: number, initValue: any): any[] => {
...
const replaceTextByWords = (
  ...
  // 空文字置換がダメな時に、空文字置換が来たら、置換前の文字列で代わりに置換する
  const replacedWord =
    !cb_wordDelete.value && replacementTable[index][1] === ""
      ? word
  ...
  if (index < replacementTable.length - 1) {
    replacedSplit = replacedSplit.map(partText => {
      return replaceTextByWords(partText, replacementTable, index + 1);
    });
  }
